...
// Initial mapping - this will be updated with backend data
...
export default function FinancialReportGenerator() {
  ...
  const [assetMapping, setAssetMapping] = useState(symbolToAssetId);
    ...
    const fetchAssets = async () => {
      try {
        ...
        if (response.data) {
          ...
          // Build a mapping of symbols to asset IDs
          const mapping = {};
          response.data.forEach(asset => {
            if (asset.symbol && asset.id) {
              mapping[asset.symbol] = asset.id;
            }
          });
          
          // Update the asset mapping
          setAssetMapping(prev => ({...prev, ...mapping}));
          console.log("Updated asset mapping:", mapping);
        }
      } catch (err) {
        console.error("Error fetching assets:", err);
        // Don't show error to user, just use the default mapping
      }
    };
  ...
  const handleSelectStock = (symbol) => {
    ...
    // Clear previous report when changing stock selection
    if (symbol !== selectedStock) {
      setMarkdownReport('');
      setError(null);
      // Find asset ID for the selected stock
      const id = assetMapping[symbol] || null;
      setAssetId(id);
    }
  };
  ...
  const handleSubmit = async () => {
    if (selectedStock) {
      ...
      try {
        // Use the mapping to get the asset ID, or fall back to a default
        const id = assetMapping[selectedStock] || 1159;
        
        const response = await BackendClient.get(`/assets/${id}/get-report/`);
        
        if (response.data && response.data.result) {
          setMarkdownReport(response.data.result);
        } else {
  ...
  const handleDownloadReport = async () => {
    if (selectedStock) {
      try {
        ...
        const response = await BackendClient.get(`/assets/${id}/download-report/`, {
          responseType: 'blob'
        });
        ...
        link.href = url;
        link.setAttribute('download', `${selectedStock}_report.pdf`);
        ...
        <Grid >
          ...
          <Grid item xs={12} md={4}>
            <Card sx={{ p: 3, mt: 4, boxShadow: theme.shadows[4] }}>
              <Typography variant="h5" gutterBottom>Financial Report Generator</Typography>
              ...
              <Box sx={{ minHeight: 40 }}>
                {selectedStock ? (
                  <Chip
                    label={selectedStock}
                    onDelete={() => setSelectedStock(null)}
                    color="primary"
                    variant="outlined"
                  />
                ) : (
          ...
          <Grid item xs={12} md={8}>
            ...
            >
              ...
              {!isLoading && !error && markdownReport ? (
                <Box sx={{ 
                  p: 2, 
                  '& img': { maxWidth: '100%' }, 
                  '& table': { border: '1px solid #ddd', borderCollapse: 'collapse' },
                ...
                <Box sx={{ 
                  ...
                  <Typography variant="h6" gutterBottom>No Report Generated</Typography>
                  <Typography variant="body2">
                    Select a stock and click "Generate Report" to view a detailed financial analysis.
                  </Typography>
                </Box>
              ) : null}
            </Paper>
